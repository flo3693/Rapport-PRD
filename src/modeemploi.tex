...
\begin{itemize}
  ...
  \item Le nom doit être descriptif de l'emplacement. Ainsi, \latexcode{chap1} et \latexcode{sec2.1} doivent absoluement être évités et remplacés par \latexcode{chap:trousnoirs} et \latexcode{sec:bigbang_theory}.
  \item
\end{itemize}
...
\begin{itemize}
  \item \latexcode{\\ref} : insère le numéro identifiant l'emplacement (partie, chapitre, section, equation\ldots).
...
Pour insérer du code dans votre document, vous avez à disposition le package \latexcode{listings}. Ce package est très riche en possibilités, il vous est fortement recommendé de consulter sa documentation si vous souhaitez l'exploiter en profondeur.

\section{Commandes simplifiées}

Pour simplifier son utilisation, plusieurs commandes et environnements supplémentaires ont été défini. Pour chacun des languages supportés, trois commandes et un environnements sont disponibles. En notant \latexcode{XXX} le nom du language, on a :
\begin{itemize}
  \item La commande \latexcode{\\XXXcode} :\par
  	\begin{itemize}
		  \item Commande identique à la commande \latexcode{\\lstinline\{ \}} mais préconfigurée pour le language \latexcode{XXX}.
  ...
  \item La commande \latexcode{\\XXXcodeintab} :\par
	...
	\item La commande \latexcode{\\XXXsourcefile} :\par
		\begin{itemize}
		  \item Commande identique à \latexcode{\\lstinputlisting} mais préconfigurée pour le language \latexcode{XXX}.
	...
	\item L'environnement \latexcode{\\begin\{XXXsource\} \\end\{XXXsource\}} :
		\begin{itemize}
		  \item Environnement identique à \latexcode{\\begin\{lstlisting\} \\end\{lstlisting\}} mais préconfigurée pour le language \latexcode{XXX}.
...
Toutes ces commandes et environnements admettent un paramètre optionel permettant de changer leur comportement. Pour le détail de ce que vous pouvez faire, consultez la documentation de \href{https://www.ctan.org/pkg/listings}{listings} sur le CTAN.
...
Les languages pour lesquels les commandes simplifiées ont été définis sont donnés par la \autoref{tab:lstlang}.

\begin{Table}{tab:lstlang}{Liste des languages avec commandes simplifiés pour le code source}
\begin{tabu}{|c|c|}
\hline
...
Le package listings supporte de nombreux languages et vous pouvez même définir les règles de mise en forme pour votre propre language (cf. manuel de listings).
Pour créer les commandes simplifiées pour un langage, il suffit d'utiliser la commande \latexcode{\\newsourceformat\{cpp\}\{C++\}}. Dans cet exemple, \latexcode{cpp} est le préfix utilisé pourles commandes et \latexcode{C++} est le nom du language tel qu'il est connu par la package listings.
...
La bibliographie est géré via bibtex. Il vous suffit de spécifier le ou les fichiers de bibliographie (fichier d'extension \latexcode{.bib}) avec la commande
\latexcode{\\bibliography\{nomdufichiersansextension\}}. Cette commande est à insérer juste avant la commande \latexcode{\\makelastpage}

Si vous renseignez le champs bibtex \latexcode{annote} alors la clé de citation et le contenu de ce champs apparaitront dans la bibliographie.
