...
L'objectif de ce projet est de réaliser une application qui, à l'aide d'une caméra, va repérer le visage d'une personne handicapée puis zoomer dessus pour ensuite analyser son émotion actuelle. En fonction de l'émotion perçue, une action spécifique sera réalisée (ex: si la personne est triste, lui mettre de la musique joyeuse). 
}{images/ptz.png}{}
\posterblock{7 émotions universelles}{
A ce jour, il a été démontré qu'il existait 7 émotions dite universelles qui sont les suivantes: la neutralité, la joie, la tristesse, la colère, la peur, la surprise et le dégoût.\\
C'est Charles Darwin en 1872 puis le psychologue Paul Ekman en 1971 qui ont défini ces émotions.
}{images/ck.jpg}{}
\posterblock{Norme de description}{
	%Plusieurs normes permettent de définir des expressions du visage et donc des émotions.\\
Pour décrire les mouvements du visage, la norme la plus connue est \textbf{FACS}, qui signifie Facial Action Coding System. Ce système décompose tous les mouvements du visage en 46 \textbf{Action Units} (AU), chacune décrivant la contraction ou la décontraction d'un ou plusieurs muscles du visage.  Par exemple un sourire, et donc l'émotion de la joie, est composé des AUs 6 (remontée des joues) et 12 (étirement du coin des lèvres).\\
...
\chapter{Nouvelle approche via un modèle probabiliste}
...
\textbf{Fonction F4: Reconnaître l'expression/Apprentissage}\\
Cette fonction sera la même que celle déjà utilisée par Vitale et al. dans leur programme Matlab.\\
\\
\textbf{Fonction F6: Reconnaître l'expression/Classification}\\
Cette fonction devra utiliser le programme Matlab de Vitale et al. déjà existant. A partir des travaux déjà réalisé, cette fonction viendra se greffer sur le programme et permettra, grâce à une image donnée en entrée, de trouver sur l'espace latent, recensant les échantillons appris lors de la phase d'apprentissage, de trouver l'émotion la plus proche et donc la plus probable.\\
